...
export function bars(items) {
  const max = Math.max(...items.map(i => i.value), 1);
  return items.map(i => `<p id="${escapeHtml(i.id || '')}"><span>${escapeHtml(i.label)}</span> <b>${escapeHtml(i.value)}</b>${i.hint ? ` <span class="muted">${escapeHtml(i.hint)}</span>` : ''}</p><div class="bar"><span style="width:${Number(i.value) / max * 100}%"></span></div>`).join('');
}
